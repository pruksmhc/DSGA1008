...
class RegionProposalNetwork(nn.Module):
    ...
    def __init__(self, in_channels, device, mid_channels = 512, n_anchor = 9, image_size = 800,
        ...
        if reg_layer  is None:
            self.reg_layer = nn.Conv2d(mid_channels, n_anchor *4, 1, 1, 0).to(self.device)
            # Regression layer
        ...
        if cls_layer is None:
            self.cls_layer = nn.Conv2d(mid_channels, n_anchor *2, 1, 1, 0).to(self.device)
    ...
    def generate_proposal_rois(self, features, anchors, train):
        ...
        x = self.conv1(features.to(self.device)) # out_map is obtained in section 1
